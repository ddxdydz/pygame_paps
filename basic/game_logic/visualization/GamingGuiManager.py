...
from basic.game_logic.game_constants import (
    GAME_GUI_IMAGES, ONE_GGUI_TICK_TO_PX, CODES_TABLE, OBJECTS_SETTINGS)
...
class GamingGuiManager:
    ...
    def get_item_image(self, obj_code, size, img_type="base"):
        ...
        name = CODES_TABLE[obj_code]
        # width, height = OBJECTS_SETTINGS[name]["size"]
        width, height = size
        path = OBJECTS_SETTINGS[name]["imgs"][img_type]
    ...
    def draw_message(self, screen):
        ...
        if self.current_time_message_showing - self.decrease_alpha_time < 0:
            text.set_alpha(int(255 * (self.current_time_message_showing / (self.time_message_showing - self.decrease_alpha_time))))
        screen.blit(text, (left_padding, upper_padding))
