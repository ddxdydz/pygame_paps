...
from basic.game_logic.layers_generator.maze.maze_solver import bfs
...
class Sceleton(GameDynamicObject):
    ...
    frames = {
        ATTACK: ["attack1", "attack2", "attack3", "attack4", "attack5", "attack6", "attack7", "attack8", "attack9", "attack10", "attack11", "attack12"],
        STAY: ["idle1", "idle2", "idle3", "idle4", "idle5", "idle6", "idle7", "idle8", "idle9", "idle10", "idle11"],
        DEAD: ["dead1", "dead2", "dead3", "dead4", "dead5", "dead6", "dead7", "dead8", "dead9", "dead10", "dead11", "dead12", "dead13", "dead14", "dead15"],
        DEATH: ["dead15"],
        WALK: ["walk1", "walk2", "walk3", "walk4", "walk5", "walk6", "walk7", "walk8", "walk9", "walk10", "walk11", "walk12", "walk13"],
    ...
    def __init__(self, coordinates, size):
        ...
        self.set_collision_rect(0.14, -0.15, 0.42, 0.55)
        ...
        self.current_attack_distance = 1.4
        self.alert_distance = 3  # ticks
        self.is_alert = False

        self.damage_area = DamageArea((0, 0), rect=(0, 0, 0.7, 0.7), damage=34)
    ...
    def process_walking(self):
        if self.current_stage != Sceleton.REACT and self.is_alert:
            ...
            if distance < 1:
                ...
                scheme_self_coordinates = Map.get_scheme_coordinates(self.get_centre_coordinates())
                scheme_obj_coordinates = Map.get_scheme_coordinates(self.target_object.get_centre_coordinates())
    ...
    def draw(self, display_manager: GamingDisplayManager):
        display_manager.draw(
            Sceleton.CODE, Sceleton.frames[self.current_stage][self.current_frame_index],
